...
def convert():
    for z in range(0,7):
        ...
        try:
            ...
            tk.Label(first, text=f"INVALID INPUT").grid(row=8, column=4)
...
#convert_button
convert_button = tk.Button( first, text="Convert", command = convert) # Todo add function
convert_button.grid(row=i+1, column=4)

#clear_button
clear_button = tk.Button( first, text="Clear", command = clear) # Todo add function
